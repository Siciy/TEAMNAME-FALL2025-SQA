...
def giveTimeStamp():
  ...
  logger.debug(f"Timestamp generated: {strToret}")
  return strToret
...
def dumpContentIntoFile(strP, fileP):
    ...
    logger.debug(f"Content length: {len(strP)} characters")
    fileToWrite = open( fileP, 'w')
    fileToWrite.write(strP )
    fileToWrite.close()
    file_size = str(os.stat(fileP).st_size)
    logger.info(f"Successfully wrote {file_size} bytes to {fileP}")
    return file_size
  
  
def makeChunks(the_list, size_):
    logger.info(f"Creating chunks from list of size {len(the_list)} with chunk size {size_}")
    chunk_count = 0
    for i in range(0, len(the_list), size_):
        chunk_count += 1
        yield the_list[i:i+size_]
    logger.debug(f"Generated {chunk_count} chunks")
        
        
def cloneRepo(repo_name, target_dir):
    logger.info(f"Attempting to clone repository: {repo_name} to {target_dir}")
